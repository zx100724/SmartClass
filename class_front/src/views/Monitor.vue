...
const fetchMonitorData = async () => {
  try {
    const response = await axios.get('http://192.168.2.130:8000/api/monitor/list/', { headers: getHeaders() });
...
const triggerFacialAttendance = async () => {
  ...
  try {
    const response = await axios.get(`http://192.168.2.130:8000/api/monitor/facial-attendance/${selectedRoom.value.id}/`);
...
const triggerAiAnalysis = async () => {
  ...
  try {
    const response = await axios.get(`http://192.168.2.130:8000/api/monitor/ai-analyze/${selectedRoom.value.id}/`);
...
const submitEvaluation = async () => {
  ...
  try {
    ...
    await axios.post('http://192.168.2.130:8000/api/records/manage/', payload, {
      headers: headers
    });
